;(function () {
  ...
  function Enemy(x, y, speed) {
    ...
    this.init = function (pathStart, pathEnd) {
      that.pathStart = pathStart;
      that.pathEnd = pathEnd;
      that.htmlElement.className = 'gunda';
      that.htmlElement.style.left = that.x + 'px';
      that.htmlElement.style.top = that.y + 'px';
      animateTileSprite(that.htmlElement, 6, 200);
    };
    this.kill = function () {
      that.htmlElement.remove();
    };
  ...
  function GameWorld(){
    ...
    var initKeyEvents = function () {
      ...
      window.onkeydown = function (event) {
        if (event.which === right && keyRestrict > 0) {
          ...
          // animateTileSprite(that.bomberMan.htmlElement, 10, 1000);
          keyRestrict = 0;
          return;
        }
        ...
        if (event.which === spaceBar) {
          if (!that.bomb.bombActive) {
            ...
            animateTileSprite(that.bomb.htmlElement, 2, 200);
      ...
      window.onkeyup = function (event) {
        ...
        // animateTileSprite(that.bomberMan.htmlElement, 0, 10);
        keyRestrict = 1;
      };
    ...
    var initClickEvents = function(){
      ...
      that.endToGame.onclick = function() {
